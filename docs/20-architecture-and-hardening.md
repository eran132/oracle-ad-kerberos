...
- [ ] **No Kerberos delegation.** Neither `svc-ora01` nor `svc-ora-ldap` needs — or should ever have — "Trust this account for delegation" set. This architecture has **no second hop performed as the end user**: alice→Oracle is a single hop, and Oracle→AD-LDAP binds as `svc-ora-ldap`'s *own* credential (not alice's forwarded ticket). AD accounts are not trusted for delegation by default, so the correct action is *leave it off and verify it stays off*. Unconstrained delegation on the Oracle SPN account would let anyone who compromises `ora01` harvest forwarded TGTs of every connecting user — a serious AD attack surface. The client `krb5.ini` `forwardable = false` (set for the JDBC `EncryptionKey` fix) additionally makes delegation *impossible* even if AD were misconfigured. Avoiding delegation is a security strength of this design, not a limitation. Verify: `Get-ADUser svc-ora01,svc-ora-ldap -Properties TrustedForDelegation,TrustedToAuthForDelegation` → both `False`.
